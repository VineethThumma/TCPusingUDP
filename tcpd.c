...
/* Function to calculate the RTT of the acknowledged packet */
...
int main ()
{
    ...
    request_address.sin_port = htons (1040);
    ...
    // Update Aux DS
    inet_aton("164.107.113.20", &(MyMessage.msg_header.sin_addr));
    ...
    printf ("Detected an event\n");
        ...
        // If the packet is from ftps
            else if (localbuffer_receive[0] == '2')
            {
                ...
                printf("Server side : %d %d %d\n", canbesent, requestedbytes, start_s);
                if(head_s != NULL)
                printf("Current head : %d %d \n", head_s->sequence_number, head_s->length);
                if(canbesent > 0){
	    ...
	    // checking if the received packet is garbled
            if(checksum_calc(localbuffer_send, test.checksum, MSS+MSGLEN+SEQNOSIZE+CHECKSUMSIZE+1, 2) != true)
                ...
                if(test.fromclient == true)
                {
                  ...
                  inet_aton("164.107.113.18", &(test.msg_header.sin_addr));
